...
impl DisplayState {
    pub fn new(buffer: *mut u8, pitch: u32, width: u32, height: u32, bpp: u8) -> Self {
        ...
        let size = (
            (width / lfb::CHAR_WIDTH) as u16,
            (height / lfb::CHAR_HEIGHT) as u16,
        );
        ...
        loop {
            {
                ...
                display.lfb.direct_lfb().draw_char(
                    cursor.pos.0 as u32 * lfb::CHAR_WIDTH,
                    cursor.pos.1 as u32 * lfb::CHAR_HEIGHT,
                    &character.fg_color,
                    &character.bg_color,
                    if self.visible {
                        character.value
...
impl LFBTerminal {
    ...
    fn print_char_at(
        display: &mut MutexGuard<DisplayState>,
        color: &mut MutexGuard<ColorState>,
        c: char,
        pos: (u16, u16),
    ) -> bool {
        display.lfb.lfb().draw_char(
            pos.0 as u32 * lfb::CHAR_WIDTH,
            pos.1 as u32 * lfb::CHAR_HEIGHT,
            &color.fg_color,
            &color.bg_color,
            c,
        ) && display.lfb.direct_lfb().draw_char(
    ...
    fn scroll_up(display: &mut MutexGuard<DisplayState>, color: &mut MutexGuard<ColorState>) {
        ...
        display.lfb.lfb().fill_rect(
            0,
            (size.1 - 1) as u32 * lfb::CHAR_HEIGHT,
            size.0 as u32 * lfb::CHAR_WIDTH,
            lfb::CHAR_HEIGHT,
            &color.bg_color,
        );
        display.lfb.flush();
    ...
    fn position(
        display: &mut MutexGuard<DisplayState>,
        cursor: &mut MutexGuard<CursorState>,
        color: &mut MutexGuard<ColorState>,
        pos: (u16, u16),
    ) {
        cursor.pos = pos;
    ...
    fn clear_screen_from_cursor(
        ...
        display.lfb.lfb().fill_rect(
            pos.0 as u32 * lfb::CHAR_WIDTH,
            pos.1 as u32 * lfb::CHAR_HEIGHT,
            (size.0 - pos.0) as u32 * lfb::CHAR_WIDTH,
            lfb::CHAR_HEIGHT,
            &color.bg_color,
        );
        ...
        display.lfb.lfb().fill_rect(
            0,
            (pos.1 + 1) as u32 * lfb::CHAR_HEIGHT,
            size.0 as u32 * lfb::CHAR_WIDTH,
            (size.1 - pos.1 - 1) as u32 * lfb::CHAR_HEIGHT,
            &color.bg_color,
        );
