...
impl Apic {
    pub fn new() -> Self {
        ...
        let madt = kernel::acpi_tables()
            .lock()
            .find_table::<Madt>()
            .unwrap_or_else(|err| panic!("MADT not available ({:?})!", err));
        let int_model = madt
            .parse_interrupt_model_in(AcpiAllocator::new(kernel::allocator()))
            .unwrap_or_else(|err| panic!("Interrupt model not found in MADT ({:?})!", err));
...

        {
            ...
            match int_model.0 {
                ...
                InterruptModel::Apic(apic_desc) => {
                    ...
                    let io_apic_desc = apic_desc
                        .io_apics
                        .get(0)
                        .unwrap_or_else(|| panic!("No IO APIC described by MADT!"));
                    ...
                    unsafe {
                        io_apic = Mutex::new(IoApic::new(io_apic_desc.address as u64));
                    }

                    let mut io_apic_locked = io_apic.lock();
                    unsafe {
                        io_apic_locked.init(io_apic_desc.global_system_interrupt_base as u8);
                    }
                    ...
                    for i in io_apic_desc.global_system_interrupt_base as u8..unsafe {
                        io_apic_locked.max_table_entry()
                    } {
    ...
    pub fn allow(&self, vector: InterruptVector) {
        ...
        unsafe {
            self.io_apic.lock().enable_irq(target);
        }
    }
