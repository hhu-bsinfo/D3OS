...
impl Thread {
    ...
    pub fn kickoff_kernel_thread() {
        ...
        unsafe {
            ...
            kernel::tss().lock().privilege_stack_table[0] =
                VirtAddr::new(thread.kernel_stack_addr() as u64);
    ...
    pub fn switch(current: &Thread, next: &Thread) {
        unsafe {
            thread_switch(
                ptr::from_ref(&current.old_rsp0) as *mut u64,
                next.old_rsp0,
                next.kernel_stack_addr() as u64,
            );
        }
    }
    ...
    pub fn kernel_stack_addr(&self) -> *const u64 {
        unsafe {
            return self
                .kernel_stack
                .as_ptr()
                .offset(((self.kernel_stack.capacity() - 1) * 8) as isize);
        }
    }
    ...
    fn switch_to_user_mode(&mut self) {
        ...
        unsafe {
            thread_user_start(self.old_rsp0);
        }
    }
}
