...
pub extern "sysv64" fn sys_share_syscall_cap(thread_id: usize, syscall_number: usize) -> isize {
    ...
    let shared_cap = if let Some(sharer_cspace) = cur_thread.cspace.invoke() {
...
pub extern "sysv64" fn sys_revoke_syscall_cap(thread_id: usize, syscall_number: usize) -> isize {
    if let Some(thread) = scheduler().thread(thread_id) {
        if let Some(mut cspace) = thread.cspace.invoke_mut() {
...
pub extern "sysv64" fn sys_share_naming_cap(thread_id: usize, rights: usize, naming_object_number: usize) -> isize {
    ...
    info!(
        "sys_share_naming_cap: called with thread_id {}, rights {}, naming_object_number {}",
        thread_id,
        rights.bits(),
        naming_object_number
    );
    let cur_thread = scheduler().current_thread();
    ...
    let shared_cap = if let Some(sharer_cspace) = cur_thread.cspace.invoke() {
        if let Some(naming_cap) = sharer_cspace.get_naming_capability(naming_object_number) {
            ...
            if naming_cap.is_none() {
                warn!("sharing naming cap: naming cap is none")
            }
...
pub extern "sysv64" fn sys_revoke_naming_cap(thread_id: usize, naming_object_number: usize) -> isize {
    info!(
        "sys_revoke_naming_cap: called with thread_id {}, naming_object_number {}",
    ...
    if thread_id == current_thread.id() {
        ...
        let Some(revoke_thread) = scheduler().thread(thread_id) else {
            return -4;
        };
        let Some(mut revoke_cspace) = revoke_thread.cspace.invoke_mut() else {
            return -3;
        };
        let Ok(cap_handle) = revoke_cspace.cap_with_same_obj(cap_to_revoke) else {
            return -2;
        };
        let Some(cap_to_revoke) = revoke_cspace.get_naming_capability_mut(cap_handle) else {
            return -1;
        };
        ...
        //     }
        // }
        if !cap_to_revoke.is_original() {
            cap_to_revoke.revoke();
        } //Revoke cap from other thread. originial can only be revoked from owner thread
        0
    }
}
