...
impl TokenContextFactory for RedirectOutTruncateTokenContextFactory {
    fn create_first(_kind: &TokenKind, _ch: char) -> TokenContext {
        TokenContext {
            ...
            cmd_pos: None,
            in_quote: None,
            error: Some(&REDIRECT_OUT_TRUNCATE_BEFORE_CMD_ERROR),
            require_cmd: false,
            require_file: true,
    ...
    fn create_after(prev_clx: &TokenContext, prev_content: &str, _kind: &TokenKind, _ch: char) -> TokenContext {
        let error = prev_clx.error.or_else(|| {
            if prev_clx.cmd_pos.is_none() {
                ...
                Some(&REDIRECT_OUT_TRUNCATE_INSTEAD_OF_FILE_ERROR)
            } else if prev_clx.has_background {
                Some(&REDIRECT_OUT_TRUNCATE_AFTER_BACKGROUND_ERROR)
            } else {
                None
            }
        });
