...
impl View {
    fn render_string(&self, text: &String, buffer: &mut Bitmap, font: Font, position: Vertex, rel_caret: Option<usize>) -> Vertex {
        ...
        while let Some(c)  = text.chars().nth(i){
            ...
            x += buffer.draw_char_scaled(x+1, y, font.scale , font.scale, font.fg_color,font.bg_color, c)*font.scale+1;
            i+=1;
        }
        if i==rel_caret.unwrap_or(usize::MAX){
            buffer.draw_line(x, y, x, y+font.char_height*font.scale, YELLOW);
    ...
    fn render_simple(&self, document: &Document, buffer: &mut Bitmap, font_scale: u32, fg_color: Color, bg_color: Color) {
        ...
        buffer.clear(bg_color);
         while let Some(c) = document.text_buffer().get_char(i) {
            ...
            x += buffer.draw_char_scaled(x+1, y, font_scale , font_scale, fg_color,bg_color, c)*font_scale+1;
            i+=1;
        }
            if i==document.caret(){
                buffer.draw_line(x, y, x, y+DEFAULT_CHAR_HEIGHT*font_scale, YELLOW);
