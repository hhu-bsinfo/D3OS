...
use crate::{efi_services_available, init_lfb, naming};
...
use crate::{acpi_tables, allocator, apic, built_info, gdt, init_acpi_tables, init_apic, init_initrd, init_pci, init_serial_port, initrd, keyboard, logger, memory, network, process_manager, scheduler, serial_port, timer, tss};
...
pub extern "C" fn start(multiboot2_magic: u32, multiboot2_addr: *const BootInformationHeader) {
    ...
    scheduler().ready(Thread::load_application(initrd().entries()
        ...
        .expect("WindowManager application not available!")
        .data(), "window_manager", &Vec::new()));

    // Create and register the 'window_manager' thread (from app image in ramdisk) in the scheduler
    // scheduler().ready(Thread::load_application(initrd().entries()
    //     .find(|entry| entry.filename().as_str().unwrap() == "window_manager")
    //     .expect("WindowManager application not available!")
    //     .data(), "window_manager", &Vec::new()));
