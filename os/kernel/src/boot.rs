...
use crate::{buffered_lfb, efi_services_available, init_lfb, naming, storage};
...
use core::ptr;
use chrono::DateTime;
use log::{debug, info, warn, LevelFilter, Log};
...
pub extern "C" fn start(multiboot2_magic: u32, multiboot2_addr: *const BootInformationHeader) {
    ...
    initrd().entries().for_each(|entry| {
        info!("Initrd entry: [{}]", entry.filename().as_str().unwrap());
    });
