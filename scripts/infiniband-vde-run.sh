...
if [ "$HOST" = "ib3" ]; then
    vde_switch -s /tmp/vde-switch-ib3.ctl -daemon
    ARGS+=(
        -netdev vde,id=net0,sock=/tmp/vde-switch-ib3.ctl
        -device rtl8139,netdev=net0,id=nic0,mac="${NIC_MAC_ADDRESS}"
    )

elif [ "$HOST" = "ib4" ]; then
    vde_switch -s /tmp/vde-switch-ib4.ctl -daemon
    ARGS+=(
        -netdev vde,id=net0,sock=/tmp/vde-switch-ib4.ctl
        -device rtl8139,netdev=net0,id=nic0,mac="${NIC_MAC_ADDRESS}"
    )
fi
